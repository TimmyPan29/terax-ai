...
BYOK. Cloud providers via `@ai-sdk/*`: **OpenAI, Anthropic, Google, xAI, Cerebras, Groq, DeepSeek, Mistral, OpenRouter**, plus **OpenAI-compatible** for any custom base URL. **OpenAI Account** and **Google Account** are separate app-server / ADC transports; **Copilot Account** is a GitHub Copilot transport (see below). Local / offline providers (key-optional, model id supplied at runtime): **LM Studio, MLX, Ollama**. Provider list in `config.ts` (`PROVIDERS`); model registry includes `DEFAULT_MODEL_ID` + `DEFAULT_AUTOCOMPLETE_MODEL`.
...
- **GitHub Copilot Auth** (`lib/copilotAuth.ts`): The `gh auth token` produces a `gho_` token whose integrator is locked to `copilot-4-cli`, denying access to Gemini models. We unlock the full model roster by performing a GitHub Device Flow using the official Copilot VS-Code Client-ID (`Iv1.b507a08c87ecfe98`) to obtain a `ghu_` token. This token is then exchanged for a session token via `/copilot_internal/v2/token`. *Crucial:* All token-exchange requests must be routed through `localProxyFetch` (to bypass browser CORS) and include a `User-Agent: GitHubCopilotChat/0.30.0` header. The user can manually trigger this Device Flow authentication directly from the Settings UI (`CliAccountCard` in `ModelsSection.tsx`). The resulting `ghu_` token is strictly saved to the OS Keychain (`copilot-ghu-token`) rather than local memory so that the independent Tauri Webviews (Settings vs. Main window) can transparently sync auth state and survive restarts. Note: the latest `gpt-5.4-mini` model drops support for the standard `/chat/completions` endpoint on the Copilot API in favor of a proprietary `/responses` stream, so we internally map requests for `gpt-5.4-mini` to `gpt-5-mini` which retains standard completions support.
- **Codex account transport** (`codex/` + Rust `modules/codex.rs`): Rust owns one lazily-started `codex app-server` process, validates an RPC allowlist, and broadcasts JSONL notifications through a Tauri channel. Codex CLI owns OAuth tokens and thread rollouts. Terax persists only the optional `codexThreadId` on each chat session and maps Codex events into AI SDK UI message parts. Do not route this provider through `buildLanguageModel`.
