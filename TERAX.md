...
- **markdown/** - markdown preview renderer (backs the `markdown` tab kind). Rendered files lazily load `MarkdownMath` for candidate math delimiters. A micromark extension recognizes `\(...\)` and `\[...\]` before CommonMark escapes; the Streamdown math plugin also supports `$$`. Only parsed math nodes reach KaTeX, after HTML sanitization; code fences remain code. KaTeX CSS/fonts are bundled locally, errors retain source, and display formulas scroll within their containers. Keep KaTeX out of the shared Streamdown chunk. Validation and measurements: `docs/architecture/markdown-math-preview.md`.
